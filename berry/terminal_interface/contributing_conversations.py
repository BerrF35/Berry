...
from berry.terminal_interface.profiles.profiles import write_key_to_profile
from berry.terminal_interface.utils.display_markdown_message import (
    display_markdown_message,
)

contribute_cache_path = os.path.join(
    os.path.expanduser("~"), ".cache", "berry", "contribute.json"
)
...
Want to contribute? Run `berry --model i` to use our free, hosted model. Conversations with this `i` model will be used for training.
...
def display_contributing_current_message():
    display_markdown_message(
        f"""
---
> This conversation will be used to train Berry's open-source language model.
"""
    )


def send_past_conversations(berry):
    past_conversations = get_all_conversations(berry)
    if len(past_conversations) > 0:
        print()
        print(
            "We are about to send all previous conversations to Berry for training an open-source language model. Please make sure these don't contain any private information. Run `berry --conversations` to browse them."
        )
        print()
        time.sleep(2)
        uh = input(
            "Do we have your permission to send all previous conversations to Berry? (y/n): "
        )
        print()
        if uh == "y":
            print("Sending all previous conversations to Berry...")
            contribute_conversations(past_conversations)
            print()


def set_send_future_conversations(berry, should_send_future):
    write_key_to_profile("contribute_conversation", should_send_future)
    display_markdown_message(
        """
> Berry will contribute conversations from now on. Thank you for your help!

To change this, run `berry --profiles` and edit the `default.yaml` profile so "contribute_conversation" = False.
"""
    )
...
def contribute_conversation_launch_logic(berry):
    contribution_cache = get_contribute_cache_contents()

    if berry.will_contribute:
        contribute_past_and_future_logic(berry, contribution_cache)
...
def contribute_past_and_future_logic(
    ...
    if not contribution_cache["asked_to_contribute_future"]:
        if user_wants_to_contribute_future():
            set_send_future_conversations(berry, True)
...
def get_all_conversations(berry) -> List[List]:
    def is_conversation_path(path: str):
        _, ext = os.path.splitext(path)
        return ext == ".json"

    history_path = berry.conversation_history_path
...
def contribute_conversations(
    ...
    url = "https://api.Berry.com/v0/contribute/"
