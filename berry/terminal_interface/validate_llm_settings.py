...
I guess it's setting up the model. So maybe this should be like, berry.llm.load() soon!!!!!!!
"""
...
def validate_llm_settings(berry):
    """
        ...
        if berry.offline:
            ...
            if berry.llm.model in [
                ...
                if (
                    ...
                    and not berry.llm.api_key
                    and not berry.llm.api_base
                ):
                    display_welcome_message_once(berry)
                    ...
                    To use another language model, run `berry --local` or consult the documentation at [docs.Berry.com](https://docs.Berry.com/language-model-setup/).
                    ...
                    if response == "berry --local":
                        print(
                            "\nType `berry --local` again to use a local language model.\n"
                        )
                        exit()
    ...
    if (
        ...
        berry.display_message(f"> Model set to `{berry.llm.model}`")
    if len(berry.messages) == 1:
        # Special message for "i {command}" usage
        # berry.display_message(f"\n*{berry.llm.model} via Berry:*")
        pass

    if berry.llm.model == "i":
        berry.display_message(
            "***Note:*** *Conversations with this model will be used to train our open-source model.*\n"
        )
    if "ollama" in berry.llm.model:
        berry.llm.load()
    return


def display_welcome_message_once(berry):
    """
