...
from ..utils.berry_dir import berry_dir
from .historical_profiles import historical_profiles

profile_dir = os.path.join(berry_dir, "profiles")
user_default_profile_path = os.path.join(profile_dir, "default.yaml")

here = os.path.abspath(os.path.dirname(__file__))
berry_default_profiles_path = os.path.join(here, "defaults")
default_profiles_paths = glob.glob(os.path.join(berry_default_profiles_path, "*"))
...
def profile(berry, filename_or_url):
    ...
    return apply_profile(berry, profile, profile_path)


def get_profile(filename_or_url, profile_path):
    # i.com/ is a shortcut for Berry.com/profiles/
    ...
    for shortcut in shortcuts:
        if filename_or_url.startswith(shortcut):
            filename_or_url = filename_or_url.replace(
                shortcut, "https://Berry.com/profiles/"
            )
    ...
    if os.path.exists(profile_path):
        with open(profile_path, "r", encoding="utf-8") as file:
            if extension == ".py":
                ...
                # Remove `from berry import berry` and `berry = Berry()`, because we handle that before the script
                tree = ast.parse(python_script)
                tree = RemoveBerry().visit(tree)
    ...
    response.raise_for_status()
    if extension == ".py":
        return {"start_script": response.text, "version": berry_VERSION}
    elif extension == ".json":
...
class RemoveBerry(ast.NodeTransformer):
    """Remove `from berry import berry` and `berry = Berry()`"""

    def visit_ImportFrom(self, node):
        if node.module == "berry":
...
def apply_profile(berry, profile, profile_path):
        ...
        if message.lower() == "y":
            ...
            if profile_path.endswith("default.yaml"):
                with open(profile_path, "r") as file:
                    text = file.read()
                text = text.replace(
                    "version: " + str(profile["version"]), f"version: {berry_VERSION}"
                )
    ...
    if "computer" in profile and "languages" in profile["computer"]:
        ...
        berry.computer.languages = [
            i
            for i in berry.computer.languages
            if i.name.lower() in [l.lower() for l in profile["computer"]["languages"]]
        ]
        del profile["computer.languages"]

    apply_profile_to_object(berry, profile)
...
You are capable of **any** task.""",
        """You are Berry, a world-class programmer that can complete
...
# safe_mode: "off"  # The safety mode (see https://docs.Berry.com/usage/safe-mode)
...
# All options: https://docs.Berry.com/settings
...
def open_storage_dir(directory):
    dir = os.path.join(berry_dir, directory)
...
def reset_profile(specific_default_profile=None):
    ...
    for default_yaml_file in default_profiles_paths:
        ...
        create_berry_directory = False

        # Make the profile directory if it does not exist
        if not os.path.exists(profile_dir):
            if not os.path.exists(berry_dir):
                create_berry_directory = True
...
def get_default_profile(specific_default_profile):
    for default_yaml_file in default_profiles_paths:
        ...
        profile_path = os.path.join(berry_default_profiles_path, filename)
...
def determine_user_version():
    # Pre 0.2.0 directory
    old_dir_pre_020 = platformdirs.user_config_dir("Berry")
    # 0.2.0 directory
    old_dir_020 = platformdirs.user_config_dir("Berry Terminal")

    if os.path.exists(berry_dir) and os.listdir(berry_dir):
        # Check if the default.yaml profile exists and has a version key
        default_profile_path = os.path.join(berry_dir, "profiles", "default.yaml")
...
def migrate_app_directory(old_dir, new_dir, profile_dir):
    ...
    # After all migrations have taken place, every yaml file should have a version listed. Sometimes, if the user does not have a default.yaml file from 0.2.0, it will not add the version to the file, causing the migration message to show every time berry is launched. This code loops through all yaml files post migration, and ensures they have a version number, to prevent the migration message from showing.
...
def migrate_user_app_directory():
    ...
    if user_version == "pre_0.2.0":
        old_dir = platformdirs.user_config_dir("Berry")
        migrate_app_directory(old_dir, berry_dir, profile_dir)

    elif user_version == "0.2.0":
        old_dir = platformdirs.user_config_dir("Berry Terminal")
        migrate_app_directory(old_dir, berry_dir, profile_dir)
