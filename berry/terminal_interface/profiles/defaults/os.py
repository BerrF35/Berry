    ...
    if user_input.lower() != "y":
        ...
        for pip_combo in [
            ...
            if pip_combo[1] == "quotes":
                command = f'{pip_combo[0]} install "berry[os]"'
            else:
                command = f"{pip_combo[0]} install berry[os]"

            berry.computer.run("shell", command, display=True)
...
berry.display_message("> `OS Control` enabled")
...
if not berry.auto_run:
    ...
    berry.display_message(screen_recording_message)
    print("")

# # FOR TESTING ONLY
# # Install Berry from GitHub
# for chunk in berry.computer.run(
#     "shell",
