...
def local_setup(berry, provider=None, model=None):
    def download_model(models_dir, models, berry):
        ...
        try:
            ...
            if filtered_models:
                ...
                berry.display_message(
                    "To view or delete downloaded local models, run `berry --local_models`\n\n"
                )
    ...
    berry.display_message(
        "\n**Berry** supports multiple local model providers.\n"
    )
    ...
    if selected_model == "LM Studio":
        berry.display_message(
            """
    To use Berry with **LM Studio**, you will need to run **LM Studio** in the background.
    ...
    """
        ...
        berry.llm.api_base = "http://localhost:1234/v1"
        berry.llm.api_key = "dummy"
        ...
        try:
            ...
            berry.llm.model = f"ollama/{model}"

            # Send a ping, which will actually load the model

            old_max_tokens = berry.llm.max_tokens
            old_context_window = berry.llm.context_window
            berry.llm.max_tokens = 1
            berry.llm.context_window = 100

            berry.computer.ai.chat("ping")

            berry.llm.max_tokens = old_max_tokens
            berry.llm.context_window = old_context_window

            berry.display_message(f"> Model set to `{model}`")
        ...
        berry.display_message(
            """
    To use Berry with **Jan**, you will need to run **Jan** in the background.
    ...
    """
        ...
        berry.llm.api_base = "http://localhost:1337/v1"
        # time.sleep(1)

        # Send a GET request to the Jan API to get the list of models
        response = requests.get(f"{berry.llm.api_base}/models")
        ...
        berry.llm.model = jan_model_name
        berry.llm.api_key = "dummy"
        ...
        if platform.system() == "Darwin":  # Check if the system is MacOS
            ...
            if result.returncode != 0:
                berry.display_message(
                    "To use Llamafile, Berry requires Mac users to have Xcode installed. You can install Xcode from https://developer.apple.com/xcode/ .\n\nAlternatively, you can use `LM Studio`, `Jan.ai`, or `Ollama` to manage local language models. Learn more at https://docs.Berry.com/guides/running-locally ."
                )
                time.sleep(3)
                raise Exception(
        ...
        models_dir = os.path.join(berry.get_berry_dir(), "models")
        ...
        if not models:
            ...
            model_path = download_model(models_dir, models, berry)
        else:
        ...
        # Set flags for Llamafile to work with berry
        berry.llm.model = "openai/local"
        berry.llm.api_key = "dummy"
        berry.llm.temperature = 0
        berry.llm.api_base = "http://localhost:8080/v1"
        berry.llm.supports_functions = False

        model_name = model_path.split("/")[-1]
        berry.display_message(f"> Model set to `{model_name}`")
    ...
    if user_ram and user_ram > 9:
        berry.llm.max_tokens = 1200
        berry.llm.context_window = 8000
    else:
        berry.llm.max_tokens = 1000
        berry.llm.context_window = 3000

    # Display intro message
    if berry.auto_run == False:
        berry.display_message(
