...
def terminal_interface(berry, message):
    ...
    # If (len(berry.messages) == 1), they probably used the advanced "i {command}" entry, so no message should be displayed.
    if (
        not berry.auto_run
        and not berry.offline
        and not (len(berry.messages) == 1)
    ):
        berry_intro_message = [
            "**Berry** will require approval before running code."
        ]

        if berry.safe_mode == "ask" or berry.safe_mode == "auto":
            if not check_for_package("semgrep"):
                berry_intro_message.append(
                    f"**Safe Mode**: {berry.safe_mode}\n\n>Note: **Safe Mode** requires `semgrep` (`pip install semgrep`)"
                )
        else:
            berry_intro_message.append("Use `berry -y` to bypass this.")

        if (
            not berry.plain_text_display
        ):  # A proxy/heuristic for standard in mode, which isn't tracked (but prob should be)
            berry_intro_message.append("Press `CTRL-C` to exit.")

        berry.display_message("\n\n".join(berry_intro_message) + "\n")
    ...
    while True:
        if interactive:
            if (
                len(berry.messages) == 1
                and berry.messages[-1]["role"] == "user"
                and berry.messages[-1]["type"] == "message"
            ):
                # They passed in a message already, probably via "i {command}"!
                message = berry.messages[-1]["content"]
                berry.messages = berry.messages[:-1]
            else:
                ### This is the primary input for Berry.
                try:
                    message = (
                        cli_input("> ").strip()
                        if berry.multi_line
        ...
        if isinstance(message, str):
            ...
            if message.startswith("%") and interactive:
                handle_magic_command(berry, message)
                continue

            # Many users do this
            if message.strip() == "berry --local":
                print("Please exit this conversation, then run `berry --local`.")
                continue
            if message.strip() == "pip install --upgrade berry":
                print(
                    "Please exit this conversation, then run `pip install --upgrade berry`."
                )
                continue

            if (
                berry.llm.supports_vision
                or berry.llm.vision_renderer != None
            ):
                ...
                if image_path:
                    # Add the text berry's message history
                    ...
                    # Pass in the image to berry in a moment
                    message = {
                        "role": "user",
                        "type": "image",
                        "format": "path",
            ...
            for chunk in berry.chat(message, display=False, stream=True):
                ...
                if chunk["type"] == "confirmation":
                    if not berry.auto_run:
                        ...
                        if not berry.safe_mode == "off":
                            if berry.safe_mode == "auto":
                                should_scan_code = True
                            elif berry.safe_mode == "ask":
                                response = input(
                        ...
                        if should_scan_code:
                            scan_code(code, language, berry)

                        if berry.plain_text_display:
                            response = input(
                        ...
                        if response.strip().lower() == "y":
                            ...
                            berry.messages[-1]["content"] = code  # Give it code
                ...
                if chunk["type"] == "message":
                    ...
                    if "end" in chunk and berry.os:
                        last_message = berry.messages[-1]["content"]
                        ...
                        berry.computer.os.notify(sanitized_message)

                        # Speak message aloud
                        if platform.system() == "Darwin" and berry.speak_messages:
                            if voice_subprocess:
                ...
                if (
                    ...
                    if (berry.os == True) and (berry.verbose == False):
                    ...
                    if (
                        berry.messages[-1].get("format") != "output"
                        or berry.messages[-1]["role"] != "computer"
                        or berry.messages[-1]["type"] != "console"
                    ):
                        ...
                        berry.messages[-1]["content"] += (
                            "\n" + extra_computer_output
                        )
                        berry.messages[-1]["content"] = berry.messages[-1][
                            "content"
                        ].strip()
                ...
                if chunk["type"] == "console":
                    ...
                    if "format" in chunk and chunk["format"] == "active_line":
                        ...
                        if berry.os and active_block.active_line != None:
                            action = ""
                            ...
                            if action.startswith("computer"):
                                ...
                                if description:
                                    berry.computer.os.notify(description)
