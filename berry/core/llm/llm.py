...
class Llm:
    ...
    def __init__(self, berry):
        ...
        # Store a reference to parent berry
        self.berry = berry
        ...
        self.vision_renderer = (
            self.berry.computer.vision.query
    ...
    def run(self, messages):
        ...
        starting with LMC messages in berry.messages, going to OpenAI compatible messages into the llm,
        ...
        if model == "i":
            model = "openai/i"
            if not hasattr(self.berry, "conversation_id"):  # Only do this once
                self.context_window = 7000
                self.api_key = "x"
                self.max_tokens = 1000
                self.api_base = "https://api.Berry.com/v0"
                self.berry.conversation_id = str(uuid.uuid4())
        ...
        if self.supports_vision:
            if self.berry.os:
                # Keep only the last two images if the berry is running in OS mode
                if len(image_messages) > 1:
                    for img_msg in image_messages[:-2]:
                        messages.remove(img_msg)
                        if self.berry.verbose:
            ...
            for img_msg in image_messages:
                if img_msg["format"] != "description":
                    ...
                    try:
                        ...
                        ocr = self.berry.computer.vision.ocr(lmc=img_msg)
                        ...
                        print(
                            "\nTo use local vision, run `pip install 'berry[local]'`.\n"
                        )
                        img_msg["format"] = "description"
                        img_msg["content"] = ""
        ...
        messages = convert_to_openai_messages(
            ...
            shrink_images=self.berry.shrink_images,
            berry=self.berry,
        )
        ...
        try:
            if self.context_window and self.max_tokens:
                ...
                try:
                    ...
                    if len(messages) == 1:
                        if self.berry.in_terminal_interface:
                            self.berry.display_message(
                                """
**We were unable to determine the context window of this model.** Defaulting to 8000.

If your model can handle more, run `berry --context_window {token limit} --max_tokens {max tokens per response}`.
...
Continuing...
        ...
        if hasattr(self.berry, "conversation_id"):
            params["conversation_id"] = self.berry.conversation_id
        ...
        if (
            self.berry.debug == True and False  # DISABLED
    ...
    def load(self):
        ...
        if self.model.startswith("ollama/"):
            ...
            self.berry.display_message("*Model configuration loaded.*\n")
    ...
    for attempt in range(attempts):
        try:
            ...
            if (
                ...
                print(
                    "LiteLLM requires an API key. Trying again with a dummy API key. In the future, if this fixes it, please set a dummy API key to prevent this message. (e.g `berry --api_key x` or `self.api_key = 'x'`)"
                )
