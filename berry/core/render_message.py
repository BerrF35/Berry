...
def render_message(berry, message):
    """
    Renders a dynamic message into a string.
    """

    previous_save_skills_setting = berry.computer.save_skills
    berry.computer.save_skills = False
    ...
    for i, part in enumerate(parts):
        ...
        if part.startswith("{{") and part.endswith("}}"):
            ...
            output = berry.computer.run(
                "python", part[2:-2].strip(), display=berry.verbose
            )
    ...
    if (
        berry.debug == True and False  # DISABLED
    ):  # debug will equal "server" if we're debugging the server specifically
        print("\n\n\nSYSTEM MESSAGE\n\n\n")
        print(rendered_message)
        print("\n\n\n")

    berry.computer.save_skills = previous_save_skills_setting
