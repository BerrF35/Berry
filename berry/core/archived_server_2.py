# This is a websocket berry, TTS and STT disabled.
...
class AsyncBerry:
    def __init__(self, berry):
        ...
        # TTS
        # if self.berry.tts == "coqui":
        #     engine = CoquiEngine()
        # elif self.berry.tts == "openai":
        #     engine = OpenAIEngine()
        ...
        self._in_keyboard_write_block = (
            False  # Tracks whether berry is trying to use the keyboard
        )
    ...
    async def input(self, chunk):
        ...
        if isinstance(chunk, bytes):
            ...
            if "start" in chunk:
                ...
                self.berry.computer.terminate()
                # Stop any code execution... maybe we should make berry.stop()?
            elif "end" in chunk:
    ...
    async def run(self):
        ...
        def generate(message):
            ...
            # berry.messages = self.active_chat_messages
            # print("🍀🍀🍀🍀GENERATING, using these messages: ", self.berry.messages)
            print("passing this in:", message)
            for chunk in self.berry.chat(message, display=False, stream=True):
                ...
                # Handle code blocks
                    ...
                    #     if self.berry.messages[-1]["content"].startswith(
                    #         "computer.keyboard.write("
                    #     ):
                    #         self.berry.messages[-1]["content"] = (
                    #             "dummy_variable = ("
                    #             + self.berry.messages[-1]["content"][
...
def server(berry, port=8000):  # Default port is 8000 if not specified
    async_berry = AsyncBerry(berry)
    ...
    async def settings(payload: Dict[str, Any]):
        for key, value in payload.items():
            print("Updating berry settings with the following:")
            print(key, value)
            if key == "llm" and isinstance(value, dict):
                for sub_key, sub_value in value.items():
                    setattr(async_berry.berry, sub_key, sub_value)
            else:
                setattr(async_berry.berry, key, value)
            ...
            async def receive_input():
                while True:
                    ...
                    if isinstance(data, bytes):
                        ...
                        await async_berry.input(data["text"])
    ...
    berry.uvicorn_server = Server(config)
    berry.uvicorn_server.run()
