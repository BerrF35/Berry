"""
This file defines the berry class.
It's the main file. `from berry import berry` will import an instance of this class.
"""
import json
import os
import threading
...
from ..terminal_interface.utils.berry_dir import berry_dir
...
class Berry:
    """
    This class (one instance is called an `berry`) is the "grand central station" of this project.
    ...
    def _streaming_chat(self, message=None, display=True):
        ...
        # Display mode actually runs berry.chat(display=False, stream=True) from within the terminal_interface.
        ...
        if message or message == "":
            ...
            #             raise Exception(
            #                 "Use a multimodal model and set `berry.llm.supports_vision` to True to handle image messages."
            #             )
        ...
        raise Exception(
            "`berry.chat()` requires a display. Set `display=True` or pass a message into `berry.chat(message)`."
        )
