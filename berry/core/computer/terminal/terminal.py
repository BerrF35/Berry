...
import os
os.environ["berry_COMPUTER_API"] = "False" # To prevent infinite recurring import of the computer API
...
class Terminal:
    ...
    def run(self, language, code, stream=False, display=False):
        ...
        if language == "python":
            if (
                ...
                and os.getenv("berry_COMPUTER_API", "True") != "False"
            ):
            ...
            # This won't work because truncated code is stored in berry.messages :/
            ...
            if False and "get_last_output()" in code:
                if "# We wouldn't want to have maximum recursion depth!" in code:
                    ...
                    code_outputs = [
                        m
                        for m in self.computer.berry.messages
                        if m["role"] == "computer"
                        and "content" in m
                        and m["content"] != ""
                    ]
    ...
    def terminate(self):
        try:
            ...
            kill_behavior = config_manager.settings["apps"].get("kill_behavior", "graceful")
        except:
            kill_behavior = "graceful"
            
        for language_name in list(self._active_languages.keys()):
            language = self._active_languages[language_name]
            if language:
                if kill_behavior == "force" and hasattr(language, "process"):
                    import psutil
                    try:
                        parent = psutil.Process(language.process.pid)
