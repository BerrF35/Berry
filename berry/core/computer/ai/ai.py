...
def fast_llm(llm, system_message, user_message):
    old_messages = llm.berry.messages
    old_system_message = llm.berry.system_message
    try:
        llm.berry.system_message = system_message
        llm.berry.messages = []
        response = llm.berry.chat(user_message)
    finally:
        llm.berry.messages = old_messages
        llm.berry.system_message = old_system_message
...
class Ai:
    ...
    def chat(self, text, base64=None):
        ...
        response = ""
        for chunk in self.computer.berry.llm.run(messages):
            if "content" in chunk:
                response += chunk.get("content")
        return response

        # Old way
        old_messages = self.computer.berry.llm.berry.messages
        old_system_message = self.computer.berry.llm.berry.system_message
        old_import_computer_api = self.computer.import_computer_api
        old_execution_instructions = (
            self.computer.berry.llm.execution_instructions
        )
        try:
            self.computer.berry.llm.berry.system_message = (
                "You are an AI assistant."
            )
            self.computer.berry.llm.berry.messages = []
            self.computer.import_computer_api = False
            self.computer.berry.llm.execution_instructions = ""

            response = self.computer.berry.llm.berry.chat(text)
        finally:
            self.computer.berry.llm.berry.messages = old_messages
            self.computer.berry.llm.berry.system_message = (
                old_system_message
            )
    ...
    def query(self, text, query, custom_reduce_query=None):
        ...
        chunks = split_into_chunks(
            text, chunk_size, self.computer.berry.llm, overlap
        )

        # (Map) Query each chunk
        responses = query_map_chunks(chunks, self.computer.berry.llm, query)

        # (Reduce) Compress the responses
        response = query_reduce_chunks(
            responses, self.computer.berry.llm, chunk_size, custom_reduce_query
        )
