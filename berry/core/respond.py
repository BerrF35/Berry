...
def respond(berry):
    ...
    while True:
        ...
        system_message = berry.system_message

        # Add language-specific system messages
        for language in berry.computer.terminal.languages:
            if hasattr(language, "system_message"):
                system_message += "\n\n" + language.system_message

        # Add custom instructions
        if berry.custom_instructions:
            system_message += "\n\n" + berry.custom_instructions

        # Add computer API system message
        if berry.computer.import_computer_api:
            if berry.computer.system_message not in system_message:
                system_message = (
                    system_message + "\n\n" + berry.computer.system_message
                )

        # Storing the messages so they're accessible in the berry's computer
        # no... this is a huge time sink.....
        # if berry.sync_computer:
        #     output = berry.computer.run(
        #         "python", f"messages={berry.messages}"
        #     )

        ## Rendering ↓
        rendered_system_message = render_message(berry, system_message)
        ## Rendering ↑
        ...
        messages_for_llm = berry.messages.copy()
        ...
        if (
            ...
            try:
                ...
                berry.display_message(
                    ...
                    **Max budget:** ${berry.max_budget}

                    Press CTRL-C then run `berry --max_budget [higher USD amount]` to proceed.
                """
                )
                break
                ...
                if (
                    ...
                    display_markdown_message(
                        ...
                        You can also use `berry --max_budget [higher USD amount]` to set a budget for your sessions.
                        """
                    )
                    raise e

                elif (
                    berry.offline == False and "not have access" in str(e).lower()
                ):
                    ...
                    if (
                        ...
                        provider_message = f"\n\nThe model '{berry.llm.model}' does not exist or is invalid. Please check the model name and try again.\n\nWould you like to try Berry's hosted `i` model instead? (y/n)\n\n  "
                    elif "groq" in error_message:
                        provider_message = f"\n\nYou do not have access to {berry.llm.model}. Please check with Groq for more details.\n\nWould you like to try Berry's hosted `i` model instead? (y/n)\n\n  "
                    else:
                        provider_message = f"\n\nYou do not have access to {berry.llm.model}. If you are using an OpenAI model, you may need to add a payment method and purchase credits for the OpenAI API billing page (this is different from ChatGPT Plus).\n\nhttps://platform.openai.com/account/billing/overview\n\nWould you like to try Berry's hosted `i` model instead? (y/n)\n\n"
                    ...
                    if response.strip().lower() == "y":
                        berry.llm.model = "i"
                        berry.display_message(f"> Model set to `i`")
                        berry.display_message(
                        ...
                        raise
                elif berry.offline and not berry.os:
                    raise
                else:
                    raise
        ...
        if berry.messages[-1]["type"] == "code":
            if berry.verbose:
                print("Running code:", berry.messages[-1])

            try:
                # What language/code do you want to run?
                language = berry.messages[-1]["format"].lower().strip()
                code = berry.messages[-1]["content"]

                if code.startswith("`\n"):
                    code = code[2:].strip()
                    if berry.verbose:
                        print("Removing `\n")
                    berry.messages[-1]["content"] = code  # So the LLM can see it.
                ...
                if (
                    ...
                    real_content = berry.messages[-1]["content"]
                    berry.messages[-1] = {
                        "role": "assistant",
                        "type": "message",
                        "content": f"```\n{real_content}\n```",
                ...
                if berry.computer.terminal.get_language(language) is None:
                ...
                code = [m for m in berry.messages if m["type"] == "code"][-1][
                    "content"
                ]

                # don't let it import computer — we handle that!
                if berry.computer.import_computer_api and language == "python":
                ...
                berry.computer.verbose = berry.verbose
                berry.computer.debug = berry.debug
                berry.computer.emit_images = berry.llm.supports_vision
                berry.computer.max_output = berry.max_output

                # sync up the berry's computer with your computer
                try:
                    if berry.sync_computer and language == "python":
                        computer_dict = berry.computer.to_dict()
                        ...
                        berry.computer.run("python", sync_code)
                except Exception as e:
                    if berry.debug:
                        raise
                    print(str(e))
                ...
                for line in berry.computer.run(language, code, stream=True):
                    yield {"role": "computer", **line}

                ## ↑ CODE IS RUN HERE

                # sync up your computer with the berry's computer
                try:
                    if berry.sync_computer and language == "python":
                        # sync up the berry's computer with your computer
                        result = berry.computer.run(
                            "python",
                            """
                            import json
            ...
            loop_message = berry.loop_message
            if berry.os:
            ...
            loop_breakers = berry.loop_breakers

            if (
                berry.loop
                and berry.messages
                and berry.messages[-1].get("role", "") == "assistant"
                and not any(
                    task_status in berry.messages[-1].get("content", "")
                ...
                berry.messages = combined_messages
