...
def scan_code(code, language, berry):
    """
    Scan code with semgrep
    """
    language_class = berry.computer.terminal.get_language(language)

    temp_file = create_temporary_file(
        code, language_class.file_extension, verbose=berry.verbose
    )
    ...
    try:
        ...
        if scan.returncode == 0:
            ...
            print(
                f"  {'Code Scanner: ' if berry.safe_mode == 'auto' else ''}No issues were found in this {language_name} code."
            )
            print("")
    ...
    cleanup_temporary_file(temp_file, verbose=berry.verbose)
