...
from berry.core.async_core import AsyncBerry, Server
...
class TestServerConstruction(TestCase):
    ...
    def test_host_and_port_passed_in(self):
        ...
        with mock.patch.dict(
            os.environ,
            {"berry_HOST": "this-is-supes-fake", "berry_PORT": "9876"},
        ):
            sboth = Server(AsyncBerry(), host, port)
    ...
    def test_host_and_port_from_env_1(self):
        ...
        with mock.patch.dict(
            os.environ,
            {"berry_HOST": fake_host, "berry_PORT": str(fake_port)},
        ):
            s = Server(AsyncBerry())
