...
#####
from berry import AsyncBerry, Berry
...
def test_hallucinations():
    ...
    for chunk in berry._respond_and_store():
...
def run_auth_server():
    os.environ["berry_REQUIRE_ACKNOWLEDGE"] = "True"
    os.environ["berry_API_KEY"] = "testing"
    async_berry = AsyncBerry()
    async_berry.print = False
    async_berry.server.run()
...
def run_server():
    os.environ["berry_REQUIRE_ACKNOWLEDGE"] = "False"
    if "berry_API_KEY" in os.environ:
        del os.environ["berry_API_KEY"]
    async_berry = AsyncBerry()
    async_berry.print = False
    async_berry.server.run()
...
def test_server():
    ...
    async def test_fastapi_server():
        ...
        async with websockets.connect("ws://localhost:8000/") as websocket:
            ...
            response = berry.computer.ai.chat(
                str(messages)
            ...
            #             "type": "image",
            #             "format": "path",
            #             "content": "/Users/jaijiteshlucas/Documents/GitHub/berry/screen.png",
            #         }
            #     )
            # )
            ...
            # actually wait i dont think this will work..? will just kill the python berry
...
def test_pytes():
    ...
    if files_on_desktop:
        ...
        ocr = berry.computer.vision.ocr(path=first_file_path)
        print(ocr)
        print("what")
    else:
...
def test_ai_chat():
    print(berry.computer.ai.chat("hi"))
...
@pytest.mark.skip(reason="Requires berry[local]")
def test_localos():
    berry.computer.emit_images = False
    berry.computer.view()
    berry.computer.emit_images = True
    assert False
...
def test_point():
    # berry.computer.debug = True
    berry.computer.mouse.move(icon="gear")
    berry.computer.mouse.move(icon="refresh")
    berry.computer.mouse.move(icon="play")
    berry.computer.mouse.move(icon="magnifying glass")
    berry.computer.mouse.move("Spaces:")
    assert False
...
def test_skills():
    ...
    # berry.computer.skills.path = skills_path
    print(berry.computer.skills.path)
    if os.path.exists(berry.computer.skills.path):
        for file in os.listdir(berry.computer.skills.path):
            os.remove(os.path.join(berry.computer.skills.path, file))
    print("Path: ", berry.computer.skills.path)
    print("Files in the path: ")
    berry.computer.run("python", "def testing_skilsl():\n    print('hi')")
    for file in os.listdir(berry.computer.skills.path):
        print(file)
    berry.computer.run("python", "def testing_skill():\n    print('hi')")
    print("Files in the path: ")
    for file in os.listdir(berry.computer.skills.path):
        print(file)

    try:
        skills = berry.computer.skills.search(query)
    except ImportError:
...
def test_browser():
    berry.computer.api_base = "http://0.0.0.0:80/v0"
    print(
        berry.computer.browser.search("When's the next Dune showing in Seattle?")
    )
    assert False
...
def test_display_api():
    ...
    # berry.computer.display.find_text("submit")
    # assert False
    ...
    for icon in icons:
        if icon.endswith("icon icon"):
            say("click the " + icon)
            berry.computer.mouse.move(icon=icon.replace("icon icon", "icon"))
        elif icon.endswith("icon"):
            say("click the " + icon)
            berry.computer.mouse.move(icon=icon.replace(" icon", ""))
        elif icon.endswith("text"):
            say("click " + icon)
            berry.computer.mouse.move(icon.replace(" text", ""))
        else:
            say("click " + icon)
            berry.computer.mouse.move(icon=icon)

    # berry.computer.mouse.move(icon="caution")
    # berry.computer.mouse.move(icon="bluetooth")
    # berry.computer.mouse.move(icon="gear")
    # berry.computer.mouse.move(icon="play button")
    # berry.computer.mouse.move(icon="code icon with '>_' in it")
...
def test_websocket_server():
    ...
    server_thread = threading.Thread(target=berry.server)
    server_thread.start()
    ...
    ws.send(
        "Hello, berry! What operating system are you on? Also, what time is it in Seattle?"
    )
...
def test_i():
    ...
    data = "Hello, berry! What operating system are you on? Also, what time is it in Seattle?"
...
def test_async():
    berry.chat("Hello!", blocking=False)
    print(berry.wait())
...
@pytest.mark.skip(reason="Computer with display only + no way to fail test")
def test_notify():
    berry.computer.os.notify("Hello")
    assert False


@pytest.mark.skip(reason="Computer with display only + no way to fail test")
def test_get_text():
    print(berry.computer.display.get_text_as_list_of_lists())
    assert False
...
def test_keyboard():
    time.sleep(2)
    berry.computer.keyboard.write("Hello " * 50 + "\n" + "hi" * 50)
    assert False
...
def test_get_selected_text():
    ...
    time.sleep(1)
    text = berry.computer.os.get_selected_text()
    print(text)
    assert False
...
def test_display_verbose():
    berry.computer.verbose = True
    berry.verbose = True
    berry.computer.mouse.move(x=500, y=500)
    assert False
...
def test_long_message():
    ...
    berry.llm.context_window = 300
    berry.chat(messages)
    assert len(berry.messages) > 1
    assert "A" in berry.messages[-1]["content"]
...
def test_spotlight():
    berry.computer.keyboard.hotkey("command", "space")
...
def test_files():
    messages = [
        {"role": "user", "type": "message", "content": "Does this file exist?"},
        {
            ...
            "type": "file",
            "format": "path",
            "content": "/Users/jaijitesh/image.png",
        },
    ]
    berry.chat(messages)
...
def test_vision():
    ...
    berry.llm.supports_vision = True
    berry.llm.model = "gpt-4o-mini"
...
def test_multiple_instances():
    berry.system_message = "i"
    agent_1 = Berry()
    agent_1.system_message = "<3"
    agent_2 = Berry()
    agent_2.system_message = "u"

    assert berry.system_message == "i"
...
def test_hello_world():
    ...
    messages = berry.chat(hello_world_message)
...
def test_math():
    ...
    print("loading")
    messages = berry.chat(order_of_operations_message)
    print("done")
...
with open('numbers.txt', 'a+') as f:                                                                                                        
    ...
    for chunk in berry.computer.run("python", code, stream=True, display=True):
        print(chunk)
...
def test_write_to_file():
    ...
    berry.messages = []  # Just reset message history, nothing else for this test
    messages = berry.chat(
...
def test_reset():
    # make sure that berry.reset() clears out the messages Array
    assert berry.messages == []


def test_token_counter():
    system_tokens = count_tokens(
        text=berry.system_message, model=berry.llm.model
    )

    prompt = "How many tokens is this?"

    prompt_tokens = count_tokens(text=prompt, model=berry.llm.model)

    messages = [
        {"role": "system", "message": berry.system_message}
    ] + berry.messages

    system_token_test = count_messages_tokens(
        messages=messages, model=berry.llm.model
    )
