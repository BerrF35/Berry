...
$packageName = "berry litellm openai"
$desktopPath = [System.IO.Path]::Combine([System.Environment]::GetFolderPath('Desktop'), 'Berry.lnk')
$condaExePath = "$condaPath\Scripts\conda.exe"

# URL of the .ico file
$icoUrl = "https://raw.githubusercontent.com/berry/berry/main/docs/assets/favicon.ico"
$icoPath = "$env:TEMP\berry.ico"
...
# Create a shortcut on the desktop to activate the environment, install berry, and run it
$targetPath = "$env:SystemRoot\System32\cmd.exe"
$arguments = "/K `"$userCondaScriptsPath\activate.bat` $userEnvName && echo Updating Berry && pip install -U $packageName && cls && echo Launching Berry && berry`""
...
Write-Host "Shortcut 'Berry.lnk' has been created on the desktop with the custom icon."
